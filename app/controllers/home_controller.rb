class HomeController < ApplicationController
  def index
    ...
    @online_green_count, @online_yellow_count, @online_red_count = Rails.cache.fetch('online_count', :expires_in => 60.seconds) do
      ...
      res = http.head("/devices/?query=#{CGI::escape ActiveSupport::JSON.encode(@online_green_query)}")
      green = res['Total'].to_i
      res = http.head("/devices/?query=#{CGI::escape ActiveSupport::JSON.encode(@online_yellow_query)}")
      yellow = res['Total'].to_i
      res = http.head("/devices/?query=#{CGI::escape ActiveSupport::JSON.encode(@online_red_query)}")
      red = res['Total'].to_i
      [green, yellow, red]
    end
  end
